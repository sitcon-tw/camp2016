...
var aboutCampText = {
	title: "關於夏令營",
	content: [
		"有鑑於資訊科技逐漸成為雲端時代的重要學門，社群與開源精神亦持續抬頭，SITCON 團隊認為將資訊教育向下扎根，為在資訊科學領域有興趣的學生提供一個親切而良好的機會入門，並播灑開源種子，是一件相當重要且值得去做的事情。經過 2014、2015 年暑假，SITCON 夏令營已將邁入第三屆，在此之前課程內容涵括開源碼的介紹、資訊安全、Maker、Python、網頁前端等，更有企業參訪、社群闖關、視界咖啡廳、黑客松等活動，讓學員充分吸收新知、尋找自己的愛好及興趣，進而加入我們一起推動 OpenSource。",
		"2016 年度，以「墜入資訊夢，喚醒開源魂」為主軸，SITCON 夏令營將於中央大學舉辦，新創開源鄉的概念，配合豐富的課程、座談會及社群闖關，讓學員在程式編寫管理、團隊合作等架構下學習；今年將更進一步把故事和營隊整合，完整營造整體感，務求讓學員擁有截然不同的營隊體驗。"
	]
};
...
var AboutCamp = React.createClass({
	...
	render: function() {
		return (
			<div className={"aboutCamp slideInPre"} ref="slideIn">
...
var AboutSitcon = React.createClass({
	...
	render: function() {
		return (
			<div className={"aboutSitcon slideInPre"} ref="slideIn">
...
var AboutDetail = React.createClass({
	...
	render: function() {
		var allDetail = this.props.details.map(function(li, cnt) {
			return (
				<div className="detailRow" key={"detailRow"+cnt} ref="slideIn">
					<h2>{li.title}</h2>
					<Article>
						{li.content}
					</Article>
		...
		return (
			<div className={"aboutDetail slideInPre"}  ref="slideIn">
				{allDetail}
			</div>
		);
	}
...
var CoC = React.createClass({
	...
	render: function() {
		return (
			<div className={"coc slideInPre"} ref="slideIn">
				<h2>{coc.title}</h2>
				<Article>
					{coc.content}
				</Article>
...
var AboutPage = React.createClass({
	render: function() {
		return (
			<div className="aboutPage pageContainer" ref="slideIn">
